...
import PreviewAnswerOption from './PreviewAnswerOption';
const PreviewAnswerBox = ({assessment, answer})=>{
    ...
    return (
        <div className='answer_box'>
             ...
             <div className='question'>{parse(answer.question)}</div>
            {
                answer.options.map((option, i)=>(
                    <PreviewAnswerOption isAnswer={option.isCorrect} wrong={wrong && answer.response === optionsI[i]} on={optionsI[i]} oc={option.option}/>
                ))
            }
        ...
        {assessment.config.previewStatus && <div style={{margin: 12, border: '1px solid #ccc', display: 'table', padding: '2px 12px', borderRadius: 8, fontSize: 16}}>
            Status: <span style={{color: answer.status==="attempted" && !wrong?"green":"red", marginRight: '8px'}}>{answer.status==="attempted"? wrong? "Wrong":"Correct":answer.status}</span>
        </div>}
        </div>
    )
}
export default PreviewAnswerBox
...
const RdModal = (props)=>{ 
    const {showModal, onQnAdded, setShowModal, isQl=false, questions, setQuestions} = props
        ...
        if(question.length<8){
           ...
           if(totalOptions === 6 && optionF !== "")
            ...
            if(!isQl){
                ...
                let qf = {question: question, question_id: question_id, topic: currentTopic, options: ops}
                let qns = [...questions]
                qns.push(qf)
                setQuestions(qns)
                hideModal()
                ...
                let qf = {question: bodySpan.outerHTML, topic: currentTopic, options: ops}        
